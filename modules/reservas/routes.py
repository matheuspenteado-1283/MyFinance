...
@bp.route('/api/reservas/posicoes', methods=['GET'])
...
@bp.route('/api/reservas/posicoes', methods=['POST'])
def api_post_posicao():
    if 'user_email' not in session:
        return jsonify({'error': 'Não logado'}), 401
    d = request.json or {}
    add_posicao(
        session['user_email'], d.get('banco'), d.get('tp_reserva'),
        d.get('moeda', 'BRL'), d.get('data_inicio'), d.get('valor_investido_inicial'),
    )
    return jsonify({'status': 'ok'})


@bp.route('/api/reservas/posicoes/<int:pos_id>', methods=['PUT'])
def api_put_posicao(pos_id):
    if 'user_email' not in session:
        return jsonify({'error': 'Não logado'}), 401
    d = request.json or {}
    update_posicao(
        pos_id, session['user_email'], d.get('banco'), d.get('tp_reserva'),
        d.get('moeda', 'BRL'), d.get('data_inicio'), d.get('valor_investido_inicial'),
    )
    return jsonify({'status': 'ok'})


@bp.route('/api/reservas/posicoes/<int:pos_id>', methods=['DELETE'])
...
@bp.route('/api/reservas/posicoes/<int:pos_id>/mensal', methods=['GET'])
...
@bp.route('/api/reservas/posicoes/<int:pos_id>/mensal', methods=['POST'])
...
@bp.route('/api/reservas/mensal/<int:snapshot_id>', methods=['DELETE'])
...
@bp.route('/api/reservas/resumo', methods=['GET'])
def api_get_resumo():
    if 'user_email' not in session:
        return jsonify({'error': 'Não logado'}), 401
    moeda_base = request.args.get('moeda_base')
    return jsonify(get_resumo(session['user_email'], moeda_base))


# ── Upload/Export em massa ──────────────────────────────────────────────────

@bp.route('/api/upload_reservas', methods=['POST'])
def api_upload_reservas():
    if 'user_email' not in session:
        return jsonify({'error': 'Não logado'}), 401
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            tp_reserva = row.get('tp_reserva', row.get('Tipo Reserva', row.get('Tipo Investimento', '')))
            pos_id = add_posicao(
                session['user_email'],
...
@bp.route('/api/export_reservas', methods=['GET'])
def api_export_reservas():
    if 'user_email' not in session:
        return jsonify({'error': 'Não logado'}), 401
    ...
    if not df_posicoes.empty:
        ...
        df_posicoes.rename(columns={
            'id': 'ID', 'banco': 'Banco', 'tp_reserva': 'Tipo Reserva', 'moeda': 'Moeda',
    ...
    return send_file(output, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                     as_attachment=True, download_name='Reservas.xlsx')
